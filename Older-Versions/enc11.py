...
# enc 11.0.1 - CREATED BY RAPIDSLAYER101 (Scott Bree)
...
def otp(amount, shift_num, alpha):  # todo redesign?
    shift_value = ""
...
def xor(plaintext, shift_num, enc):
    if enc:
...
def encrypt_block(enc, data, block_seed, alpha, send_end=None):
    ...
    if enc.lower() in ["e", "en", "enc", "encrypt"]:
        if type(data) != bytes:
            data = data.encode()
        block = xor(data, shift_num, True)
    else:
        data = decompress(data)
        block = xor(data, shift_num, False)
        try:
            block = block.decode()
    ...
    if enc.lower() in ["e", "en", "enc", "encrypt", "d", "de", "dec", "decrypt"]:
        ...
        if len(text) == 1:
            ...
            if enc.lower() in ["e", "en", "enc", "encrypt"]:
                if type(text) != bytes:
                    text = text.encode()
                return xor(text, otp(len(text), str(shift_seed), alpha), True)
            else:
                text = decompress(text)
                output_end = xor(text, otp(len(text), str(shift_seed), alpha), False)
                try:
