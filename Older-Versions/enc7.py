...
def pass_to_seed(password, salt=None):
    if salt:
        salt = sha512(sha512(base64.b85encode(salt.encode())).hexdigest().encode()).hexdigest()
    else:
        with open("rdisc.salt") as f:
            salt = sha512(sha512(base64.b85encode(f.read().encode())).hexdigest().encode()).hexdigest()
...
def seed_to_data(seed, salt=None):
    seed = int(seed, 36)
    if salt:
        seed2 = pass_to_seed(str(seed), salt)
    else:
...
def encrypt_key(text, key, salt=None):
    if salt:
        alpha1, alpha2, num2, seed3 = seed_to_data(pass_to_seed(key, salt), salt)
    else:
...
def decrypt_key(e_text, key, salt=None):
    if salt:
        alpha1, alpha2, num2, seed3 = seed_to_data(pass_to_seed(key, salt), salt)
    else:
        alpha1, alpha2, num2, seed3 = seed_to_data(pass_to_seed(key))
